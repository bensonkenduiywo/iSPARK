...
# ============================================
# Famine Early Warning Systems Network (FEWS NET) Land Data Assimilation System (FLDAS) Monthly Soil Mositure SoilMoi00_10cm_tavg
# ============================================
soil = (
...
def monthly_composites(collection, start_year, end_year, month_start, month_end, mask=None):
    """Compute monthly mean soil moisture composites between given months."""
    images = []
...
soil_monthly = monthly_composites(soil, start_year, end_year,
                                   season_start, season_end)
# ============================================
# CHECK RESULTS
# ============================================
print(f'Soil moisture {start_year} – {end_year} composites:', soil_monthly.size().getInfo(), flush=True)

# ============================================
# Stack images by renaming bands with month index
# ============================================
# Convert ImageCollection to list
imageList = soil_monthly.toList(soil_monthly.size())
nMonths = soil_monthly.size().getInfo()
...
bandNames0 = first_img.bandNames().map(
    lambda b: ee.String('SM_').cat(year0).cat('_').cat(month0)
)
...
for i in range(1, nMonths):
    ...
    newNames = img.bandNames().map(
        lambda b: ee.String('SM_').cat(year).cat('_').cat(monthIndex)
    )
...
try:
    ...
    print("Existing asset deleted to allow overwrite.", flush=True)
except Exception as e:
    print("No existing asset to delete (safe to proceed).", flush=True)
...
while export_task.active():
    print("Export in progress... waiting 30s...", flush=True)
    time.sleep(30)
