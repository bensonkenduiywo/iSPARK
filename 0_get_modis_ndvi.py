...
#filename = f'MODIS_NDVI_{start_year}_{transition_year-1}'
filename = f'MODIS_NDVI_{start_year}_{end_year}'
...
# ============================================
# MODIS Monthly NDVI (terra: MOD13A3 and aqua: MYD13A3
# ============================================
terra = (
    ee.ImageCollection("MODIS/061/MOD13A3")
    .filterDate(ee.Date.fromYMD(start_year, season_start, 1),
				ee.Date.fromYMD(end_year, season_end, 31))
                #ee.Date.fromYMD(transition_year - 1, season_end, 31))
    .map(lambda img: (
        img.select('NDVI').divide(10000)
        .updateMask(img.select('SummaryQA').bitwiseAnd(3).lte(1))
...
def monthly_composites(collection, start_year, end_year, month_start, month_end, mask=None):
    """Compute monthly mean NDVI composites between given months."""
    images = []
...
bandNames0 = first_img.bandNames().map(
    lambda b: ee.String('NDVI_').cat(year0).cat('_').cat(month0)
)
...
for i in range(1, nMonths):
    ...
    newNames = img.bandNames().map(
        lambda b: ee.String('NDVI_').cat(year).cat('_').cat(monthIndex)
    )
